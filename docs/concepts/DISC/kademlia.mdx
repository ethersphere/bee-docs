...
In Swarm's version of Kademlia, nodes are grouped into [neighborhoods](./neighborhoods.md) of nodes based on PO (ie., neighborhood are composed of nodes which all share the same leading binary prefix bits). Each neighborhood of nodes is responsible for storing the same set of chunks. 
