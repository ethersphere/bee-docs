...
It is possible to instruct your node never to delete certain chunks, by locally pinning the chunks. Locally pinning chunks means that you will always be able to access these chunks via your node. It does mean that those chunks will be available in the network.
...
:::info
As a global pinner, you don't need to advertise your full overlay address. You need to advertise the first n characters of your overlay address, where n increases if the number of nodes in the network increases.
:::
...
You should now retry to download the chunk (possibly with another target, in case your first target didn't process the request). 
